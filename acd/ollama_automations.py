...
from acd import list_files

def ollama_with_files(prompt: str, base_url: str = "http://10.3.1.174:11434/api", model: str = "gpt-oss:120b", files: list = None) -> str:
    api = GenerateAPI(base_url=base_url, model=model)
    file_context = "\n<files_context>"
    for file in files:
    ...
    return str(result.response).strip().replace("```xml", "").replace("```", "").strip()
