        ...
        for root, _, files in walk(directory):
            for file_ in files:
                if file_matches(file_, ext_list, reg_ex):
                    file_list.append(clean_path(join(root, file_)) if path else file_)
            ...
            if reg_ex is not None:
                if ext_list is not None:
                    if path and any(file_.endswith(ext) for ext in ext_list) and search(reg_ex, file_) is not None:
                        file_list += [clean_path(join(root, file_))]
                        ind += 1
                        if qt_window:
                        ...
                        file_list += [clean_path(file_)]
                        ind += 1
                        if qt_window:
                            progress.emit((ind % 2) * multiplier + progress_start)
                elif search(reg_ex, file_) is not None:
                    file_list += [clean_path(join(root, file_))] if path else [clean_path(file_)]
                    ind += 1
                    if qt_window:
                        progress.emit((ind % 2) * multiplier + progress_start)
            else:
