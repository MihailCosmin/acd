print("acd intialization start")
import importlib
import sys
import subprocess

from os import environ
from os import pathsep
from os.path import join
from os.path import dirname
from os.path import abspath

from sys import platform

current_file_dir = dirname(abspath(__file__))
ccache_dir = join(current_file_dir, "3rd", "ccache-4.11.3-windows-x86_64")

# Only touch PATH when on Windows and when the directory exists
if platform.startswith("win"):
    try:
        import os
        if os.path.isdir(ccache_dir):
            environ["PATH"] = ccache_dir + os.pathsep + environ.get("PATH", "")
            environ["CCACHE"] = join(ccache_dir, "ccache.exe")
    except Exception:
        pass  # best-effort, no crashes at import

# Always ok to set cache home early (cheap, no network/filesystem churn)
environ.setdefault("PADDLE_PDX_CACHE_HOME", join(current_file_dir, "3rd"))
_cache = {}

__all__ = [
    'ADT_REGEX',
    'AtaNumbering',
    'BUNDLED_BREX_DIR',
    'BUNDLED_CATALOG',
    'BUNDLED_ENT_DIR',
    'BUNDLED_XSD_DIR',
    'BrexChecker',
    'BrexNotFound',
    'BrexViolation',
    'CGM2CLEARCGM',
    'CGM2SVG',
    'CHECK_LAYERS',
    'CHECK_RULES',
    'COUNTRY_IDS',
    'ConsumablesList',
    'ConsumablesValidator',
    'CsnChecker',
    'DEFAULT_BREX_DIR',
    'DELAY_DICT',
    'DELIVERY_LIST_ITEM_REGEX',
    'DM_ADDRESS_REGEX',
    'DM_REF_REGEX',
    'DictError',
    'ENCODINGS',
    'ErrorHandler',
    'ExcelReport',
    'FCChecker',
    'FILEPATH',
    'FUNC_DICT',
    'GREEK_CHARS',
    'GraphicRefChecker',
    'HtmlReport',
    'IMG_EXT',
    'INKSCAPE',
    'IPLChecker',
    'IPLNOM_REGEX',
    'ITEMDATA_REGEX',
    'ITEMNBR_REGEX',
    'ITEMNUMBER_VALUES_REGEX',
    'KWD_REGEX',
    'LINE_TYPES',
    'MFR_REGEX',
    'NO_ITEMNUMBER_VALUES_REGEX',
    'NO_PROMPT_ENV',
    'NS_DICT',
    'NoBrexDefined',
    'NoExcelSet',
    'NoOriginalTableFound',
    'NoSchemaDeclared',
    'NoXmlSet',
    'OLD_TO_NEW',
    'OR_ITEMNUMBER_VALUES_REGEX',
    'PACKAGE_DIR',
    'PAGEBLOCKS',
    'PNR_REGEX',
    'Punctuation',
    'REPORT_HTML_CSS',
    'REPORT_HTML_JS',
    'REPORT_PALETTE',
    'RES_DIR_ENV',
    'RefChecker',
    'RepairSteps',
    'S1000D_VERSION_REGEX',
    'SNS_MODES',
    'SVG_ELEMENT_REGEX',
    'SVG_HEIGHT_REGEX',
    'SVG_WIDTH_REGEX',
    'TIFF_COMPRESSION',
    'TORQUE_VALUES_REGEX',
    'TorqueValuesValidator',
    'UiLoader',
    'UnitTable',
    'UnrecognizedUnit',
    'VENDOR_EXCEPTIONS',
    'VER_REGEX_1',
    'VendorList',
    'WORD_EXTENSIONS',
    'WORD_NS',
    'Worker',
    'WorkerSignals',
    'XPATH_VERSIONS',
    'XSI_NAMESPACE',
    'XmlChecker',
    'XmlSchemaValidator',
    '__check_line_widths',
    '_as_float',
    '_ask_qt',
    '_ask_tk',
    '_filter_widgets',
    '_has_display',
    '_initialize_paddle_ocr',
    '_ipl_to_dict_excel',
    '_is_region_match',
    '_looks_like_resource_root',
    '_match_is_in_region',
    '_ngrams',
    '_normalize',
    '_phrase_to_regex',
    '_qt_application',
    '_subdirectories',
    '_version_token',
    '_walk_up_for_res',
    'add_filename_version',
    'add_iplnom_to_smg',
    'add_iplnom_to_stp',
    'add_leading',
    'adjust_column_widths',
    'ask_for_folder',
    'baselineReportFilter',
    'batch',
    'batch_check_line_widths',
    'batch_check_text_format',
    'batch_remove_repeated_header_footer',
    'blueprint',
    'brex_dirs',
    'calculate_image_similarity',
    'calculate_points',
    'catalog_path',
    'ccache_dir',
    'cgm2svg',
    'cgm2svgclear',
    'check_brackets',
    'check_cgm_details',
    'check_icn',
    'check_icns',
    'check_illu_text',
    'check_line_widths',
    'check_text_format',
    'check_tif_details',
    'chromedriver',
    'clean_path',
    'clean_word',
    'clean_xml_tags',
    'clean_xpath',
    'clearCGM2SVG',
    'collect_csdb_schemas',
    'colum_number_to_letter',
    'columns',
    'combinations_of_columns',
    'compress_img',
    'cons_and_teds_checker',
    'convert_hex_to_rgb',
    'convert_image_to_pdf',
    'convert_rgb_keys_to_hex',
    'convert_rgb_to_hex',
    'convert_sizes_to_mm',
    'copy_files',
    'copy_pdf_column',
    'count_lines_of_functions',
    'crop_image',
    'current_file_dir',
    'default_brex_dmc',
    'default_brex_path',
    'default_error_handler',
    'delete_filename_version',
    'delete_first_line',
    'detect_repeated_chunks',
    'docx_content_replace',
    'docx_footer_replace',
    'docx_header_replace',
    'download_excel',
    'driver',
    'ensure_paddle',
    'error_context',
    'estimate_illustration',
    'excel_value',
    'extract_rows_from_page',
    'fill',
    'find_characters',
    'find_default_brex_fallback',
    'find_document_by_reference',
    'find_resource_root',
    'find_schema',
    'format_excel',
    'fourDigits',
    'getContent',
    'get_2and3_refs',
    'get_4plus_refs',
    'get_average_color',
    'get_brex_ref',
    'get_chrome_driver_version',
    'get_ddn',
    'get_dm_code_from_filename',
    'get_dm_code_from_xml',
    'get_dm_codes_from_dir',
    'get_excel_sheet_names',
    'get_extensions',
    'get_file_size',
    'get_font_types',
    'get_footer_type',
    'get_manual_series',
    'get_object_attributes',
    'get_object_methods',
    'get_ocr_pdf_content',
    'get_pdf_content',
    'get_pdf_metadata',
    'get_references',
    'get_regex_string',
    'get_s1000d_refs',
    'get_s1000d_version',
    'get_schema_from_xml',
    'get_svg_data',
    'get_table_column_widths',
    'get_template_version',
    'get_textfile_content',
    'get_xml_attribute',
    'get_xml_tag_content',
    'glb2dracoglb',
    'html1',
    'html2',
    'illu_date_check',
    'illustrationChecker',
    'include_search_bar',
    'increase_filename_version',
    'increase_issue_number',
    'ipl_to_dict',
    'is_fullpage_illu',
    'is_in_range',
    'is_in_set',
    'linePrepend',
    'linearize_xml',
    'list_files',
    'list_files2',
    'list_files3',
    'load_ui',
    'make_library',
    'merge_pdfs',
    'missing_stroke_colors',
    'negative',
    'ocr_image',
    'ocr_pdf',
    'ollama_with_files',
    'ollama_with_images',
    'oneDigit',
    'option',
    'pdf2raster',
    'pdf_date_to_format',
    'pdf_page_count',
    'pdf_page_to_img',
    'pdf_to_dictionary',
    'pdfs_to_txts',
    'pgblk_9000_ted_checker',
    'prepare_estimation',
    'preprocess_svg',
    'prettyPrint',
    'prompting_enabled',
    'raster2pdf',
    'rbg2hex',
    'read_description',
    'read_dmodule',
    'read_procedure',
    'read_word_footers',
    'ref_dict_to_dm_code_dict',
    'ref_dict_to_str',
    'ref_dict_to_xml',
    'register_catalog',
    'remove_repeated_header_footer',
    'rename_illustrations',
    'replace_copyright',
    'replace_media',
    'replace_special_characters',
    'require_paddle',
    'resize_img',
    'sanitize_entity_system_uris',
    'schema_dirs',
    'send_email_with_attachments',
    'service',
    'set_inwork',
    'set_prompting',
    'set_xml_attribute',
    'set_xml_tag_content',
    'seven_unzip',
    'simple_pretty_print',
    'startTime',
    'ste_dict_rows',
    'string_similarity',
    'stroke',
    'strokeWidth',
    'svg2jpg',
    'svg2pdf',
    'svgCircle',
    'svgPolyline',
    'svgText',
    'threeDigits',
    'to_256',
    'translate_xsd_regex_to_python',
    'twoDigits',
    'unarchive_file',
    'unit_list',
    'update_chrome_driver',
    'update_filename_version',
    'update_footer_table_widths',
    'validate_ddn',
    'validate_references',
    'validate_word',
    'with_email_on_error',
    'word2pdf',
    'word_frequency',
    'zip_excel_folder',
    'zip_folder',
    'zip_word_folder',
    'zipdir',
]

def __getattr__(name):
    if name in _cache:
        return _cache[name]
    modules = {
        'ADT_REGEX': 'constants',
        'AtaNumbering': 'ataispec2200',
        'BUNDLED_BREX_DIR': 'resources',
        'BUNDLED_CATALOG': 'resources',
        'BUNDLED_ENT_DIR': 'resources',
        'BUNDLED_XSD_DIR': 'resources',
        'BrexChecker': 'brex_checker',
        'BrexNotFound': 'brex_checker',
        'BrexViolation': 'brex_checker',
        'CGM2CLEARCGM': 'cgm2clearcgm',
        'CGM2SVG': 'cgm2svg',
        'CHECK_LAYERS': 'xml_checker',
        'CHECK_RULES': 'xml_checker',
        'COUNTRY_IDS': 'vendor_list',
        'ConsumablesList': 'consTableValidator',
        'ConsumablesValidator': 'ataispec2200',
        'CsnChecker': 'reference_checker',
        'DEFAULT_BREX_DIR': 'default_brex',
        'DELAY_DICT': 'extract_rows',
        'DELIVERY_LIST_ITEM_REGEX': 'constants',
        'DM_ADDRESS_REGEX': 'constants',
        'DM_REF_REGEX': 'constants',
        'DictError': 'consTableValidator',
        'ENCODINGS': 'txt',
        'ErrorHandler': 'error_handler',
        'ExcelReport': 'report',
        'FCChecker': 'fits_and_clearences_checker',
        'FILEPATH': 'xml_validation',
        'FUNC_DICT': 'constants',
        'GREEK_CHARS': 'txt',
        'GraphicRefChecker': 'reference_checker',
        'HtmlReport': 'report',
        'IMG_EXT': 'constants',
        'INKSCAPE': 'svg2pdf',
        'IPLChecker': 'procedure_checker',
        'IPLNOM_REGEX': 'constants',
        'ITEMDATA_REGEX': 'constants',
        'ITEMNBR_REGEX': 'constants',
        'ITEMNUMBER_VALUES_REGEX': 'constants',
        'KWD_REGEX': 'constants',
        'LINE_TYPES': 'clearcgm2svg',
        'MFR_REGEX': 'constants',
        'NO_ITEMNUMBER_VALUES_REGEX': 'constants',
        'NO_PROMPT_ENV': 'prompt',
        'NS_DICT': 'brex_checker',
        'NoBrexDefined': 'brex_checker',
        'NoExcelSet': 'consTableValidator',
        'NoOriginalTableFound': 'consTableValidator',
        'NoSchemaDeclared': 'brex_checker',
        'NoXmlSet': 'ataispec2200',
        'OLD_TO_NEW': 'constants',
        'OR_ITEMNUMBER_VALUES_REGEX': 'constants',
        'PACKAGE_DIR': 'resources',
        'PAGEBLOCKS': 'estimation',
        'PNR_REGEX': 'constants',
        'Punctuation': 'xml_validation',
        'REPORT_HTML_CSS': 'report',
        'REPORT_HTML_JS': 'report',
        'REPORT_PALETTE': 'report',
        'RES_DIR_ENV': 'resources',
        'RefChecker': 'reference_checker',
        'RepairSteps': 'repair_steps',
        'S1000D_VERSION_REGEX': 'constants',
        'SNS_MODES': 'brex_checker',
        'SVG_ELEMENT_REGEX': 'estimation',
        'SVG_HEIGHT_REGEX': 'estimation',
        'SVG_WIDTH_REGEX': 'estimation',
        'TIFF_COMPRESSION': 'constants',
        'TORQUE_VALUES_REGEX': 'constants',
        'TorqueValuesValidator': 'ataispec2200',
        'UiLoader': 'utils',
        'UnitTable': 'unit_table',
        'UnrecognizedUnit': 'unit_table',
        'VENDOR_EXCEPTIONS': 'vendor_list',
        'VER_REGEX_1': 'filename_version',
        'VendorList': 'vendor_list',
        'WORD_EXTENSIONS': 'docx_',
        'WORD_NS': 'docx_',
        'Worker': 'multi',
        'WorkerSignals': 'multi',
        'XPATH_VERSIONS': 'brex_checker',
        'XSI_NAMESPACE': 'xml_processing',
        'XmlChecker': 'xml_checker',
        'XmlSchemaValidator': 'xml_validation',
        '__check_line_widths': 'svg_checks',
        '_as_float': 'xml_processing',
        '_ask_qt': 'prompt',
        '_ask_tk': 'prompt',
        '_filter_widgets': 'search_bar',
        '_has_display': 'prompt',
        '_initialize_paddle_ocr': 'ocr',
        '_ipl_to_dict_excel': 'ataispec2200',
        '_is_region_match': 'ocr',
        '_looks_like_resource_root': 'resources',
        '_match_is_in_region': 'ocr',
        '_ngrams': 'ocr',
        '_normalize': 'ocr',
        '_phrase_to_regex': 'ocr',
        '_qt_application': 'prompt',
        '_subdirectories': 'resources',
        '_version_token': 'resources',
        '_walk_up_for_res': 'resources',
        'add_filename_version': 'filename_version',
        'add_iplnom_to_smg': 'smg',
        'add_iplnom_to_stp': 'stp',
        'add_leading': 'txt',
        'adjust_column_widths': 'docx_',
        'ask_for_folder': 'prompt',
        'baselineReportFilter': 'illustrations_checker',
        'batch': 'batcher',
        'batch_check_line_widths': 'svg_checks',
        'batch_check_text_format': 'svg_checks',
        'batch_remove_repeated_header_footer': 'ocr',
        'blueprint': 'graphics',
        'brex_dirs': 'resources',
        'calculate_image_similarity': 'compare_raster',
        'calculate_points': 'clearcgm2svg',
        'catalog_path': 'resources',
        'ccache_dir': 'pdf',
        'cgm2svg': 'cgm2svg',
        'cgm2svgclear': 'cgm2clearcgm',
        'check_brackets': 'python_func',
        'check_cgm_details': 'illustration_checks',
        'check_icn': 'svg_checks',
        'check_icns': 'svg_checks',
        'check_illu_text': 'svg_checks',
        'check_line_widths': 'svg_checks',
        'check_text_format': 'svg_checks',
        'check_tif_details': 'illustration_checks',
        'chromedriver': 'get_inspect_element_html',
        'clean_path': 'filepath',
        'clean_word': 'data_extraction',
        'clean_xml_tags': 'repair_steps',
        'clean_xpath': 'brex_checker',
        'clearCGM2SVG': 'clearcgm2svg',
        'collect_csdb_schemas': 's1000d',
        'colum_number_to_letter': 'excel_',
        'columns': 'combinations',
        'combinations_of_columns': 'combinations',
        'compress_img': 'graphics',
        'cons_and_teds_checker': 'ataispec2200',
        'convert_hex_to_rgb': 'svg_checks',
        'convert_image_to_pdf': 'raster2pdf',
        'convert_rgb_keys_to_hex': 'svg_checks',
        'convert_rgb_to_hex': 'svg_checks',
        'convert_sizes_to_mm': 'svg_checks',
        'copy_files': 'copying',
        'copy_pdf_column': 'extract_rows',
        'count_lines_of_functions': 'python_func',
        'crop_image': 'graphics',
        'current_file_dir': 'pdf',
        'default_brex_dmc': 'default_brex',
        'default_brex_path': 'default_brex',
        'default_error_handler': 'error_handler',
        'delete_filename_version': 'filename_version',
        'delete_first_line': 'xml_processing',
        'detect_repeated_chunks': 'ocr',
        'docx_content_replace': 'docx_',
        'docx_footer_replace': 'docx_',
        'docx_header_replace': 'docx_',
        'download_excel': 'excel_',
        'driver': 'get_inspect_element_html',
        'ensure_paddle': 'paddle_runtime',
        'error_context': 'error_handler',
        'estimate_illustration': 'estimation',
        'excel_value': 'report',
        'extract_rows_from_page': 'extract_rows',
        'fill': 'clearcgm2svg',
        'find_characters': 'txt',
        'find_default_brex_fallback': 'default_brex',
        'find_document_by_reference': 's1000d',
        'find_resource_root': 'resources',
        'find_schema': 'resources',
        'format_excel': 'excel_',
        'fourDigits': 'clearcgm2svg',
        'getContent': 'clearcgm2svg',
        'get_2and3_refs': 's1000d',
        'get_4plus_refs': 's1000d',
        'get_average_color': 'graphics',
        'get_brex_ref': 's1000d',
        'get_chrome_driver_version': 'vendor_list',
        'get_ddn': 's1000d',
        'get_dm_code_from_filename': 's1000d',
        'get_dm_code_from_xml': 's1000d',
        'get_dm_codes_from_dir': 's1000d',
        'get_excel_sheet_names': 'excel_',
        'get_extensions': 'filelist',
        'get_file_size': 'file_info',
        'get_font_types': 'clearcgm2svg',
        'get_footer_type': 'docx_',
        'get_manual_series': 'make_library',
        'get_object_attributes': 'python_func',
        'get_object_methods': 'python_func',
        'get_ocr_pdf_content': 'ocr',
        'get_pdf_content': 'pdf',
        'get_pdf_metadata': 'pdf',
        'get_references': 's1000d',
        'get_regex_string': 'docx_',
        'get_s1000d_refs': 's1000d',
        'get_s1000d_version': 's1000d',
        'get_schema_from_xml': 'xml_processing',
        'get_svg_data': 'svg_data',
        'get_table_column_widths': 'docx_',
        'get_template_version': 'docx_',
        'get_textfile_content': 'txt',
        'get_xml_attribute': 'xml_processing',
        'get_xml_tag_content': 'xml_processing',
        'glb2dracoglb': 'glb2dracoglb',
        'html1': 'get_inspect_element_html',
        'html2': 'get_inspect_element_html',
        'illu_date_check': 'illustration_checks',
        'illustrationChecker': 'illustrations_checker',
        'include_search_bar': 'search_bar',
        'increase_filename_version': 'filename_version',
        'increase_issue_number': 's1000d',
        'ipl_to_dict': 'ataispec2200',
        'is_fullpage_illu': 'estimation',
        'is_in_range': 'xml_processing',
        'is_in_set': 'xml_processing',
        'linePrepend': 'clearcgm2svg',
        'linearize_xml': 'xml_processing',
        'list_files': 'filelist',
        'list_files2': 'filelist',
        'list_files3': 'filelist',
        'load_ui': 'utils',
        'make_library': 'make_library',
        'merge_pdfs': 'pdf',
        'missing_stroke_colors': 'svg_checks',
        'negative': 'graphics',
        'ocr_image': 'ocr',
        'ocr_pdf': 'ocr',
        'ollama_with_files': 'ollama_automations',
        'ollama_with_images': 'ollama_automations',
        'oneDigit': 'clearcgm2svg',
        'option': 'get_inspect_element_html',
        'pdf2raster': 'pdf2raster',
        'pdf_date_to_format': 'time',
        'pdf_page_count': 'pdf',
        'pdf_page_to_img': 'extract_rows',
        'pdf_to_dictionary': 'extract_rows',
        'pdfs_to_txts': 'ocr',
        'pgblk_9000_ted_checker': 'ataispec2200',
        'prepare_estimation': 'estimation',
        'preprocess_svg': 'clearcgm2svg',
        'prettyPrint': 'clearcgm2svg',
        'prompting_enabled': 'prompt',
        'raster2pdf': 'raster2pdf',
        'rbg2hex': 'clearcgm2svg',
        'read_description': 's1000d',
        'read_dmodule': 's1000d',
        'read_procedure': 's1000d',
        'read_word_footers': 'docx_',
        'ref_dict_to_dm_code_dict': 's1000d',
        'ref_dict_to_str': 's1000d',
        'ref_dict_to_xml': 's1000d',
        'register_catalog': 'resources',
        'remove_repeated_header_footer': 'ocr',
        'rename_illustrations': 'batcher',
        'replace_copyright': 'docx_',
        'replace_media': 'docx_',
        'replace_special_characters': 'xml_processing',
        'require_paddle': 'paddle_runtime',
        'resize_img': 'graphics',
        'sanitize_entity_system_uris': 'xml_processing',
        'schema_dirs': 'resources',
        'send_email_with_attachments': 'send_email',
        'service': 'get_inspect_element_html',
        'set_inwork': 's1000d',
        'set_prompting': 'prompt',
        'set_xml_attribute': 'xml_processing',
        'set_xml_tag_content': 'xml_processing',
        'seven_unzip': 'archive',
        'simple_pretty_print': 'python_func',
        'startTime': 'extract_rows_buildexe_global',
        'ste_dict_rows': 'extract_rows',
        'string_similarity': 'txt',
        'stroke': 'clearcgm2svg',
        'strokeWidth': 'clearcgm2svg',
        'svg2jpg': 'svg2jpg',
        'svg2pdf': 'svg2pdf',
        'svgCircle': 'clearcgm2svg',
        'svgPolyline': 'clearcgm2svg',
        'svgText': 'clearcgm2svg',
        'threeDigits': 'clearcgm2svg',
        'to_256': 'graphics',
        'translate_xsd_regex_to_python': 'xml_processing',
        'twoDigits': 'clearcgm2svg',
        'unarchive_file': 'archive',
        'unit_list': 'unit_list',
        'update_chrome_driver': 'vendor_list',
        'update_filename_version': 'filename_version',
        'update_footer_table_widths': 'docx_',
        'validate_ddn': 's1000d',
        'validate_references': 's1000d',
        'validate_word': 'txt',
        'with_email_on_error': 'error_handler',
        'word2pdf': 'docx_',
        'word_frequency': 'txt',
        'zip_excel_folder': 'archive',
        'zip_folder': 'archive',
        'zip_word_folder': 'archive',
        'zipdir': 'archive',
    }
    if name in modules:
        module = importlib.import_module(f'.{modules[name]}', __package__)
        value = getattr(module, name)
        _cache[name] = value
        return value
    raise AttributeError(f'module {__name__} has no attribute {name}')

print("acd intialization is finished")