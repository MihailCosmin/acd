...
def add_iplnom_to_smg(
    ...
    try:
        try:
            with open(clean_path(join(smg_dir, "product.smgXml")), "r", encoding="utf-8") as smg_xxml:
                smg_content = smg_xxml.read()
        except FileNotFoundError:
            try:
                with open(clean_path(join(smg_dir, filename, "product.smgxml")), "r", encoding="utf-8") as smg_xxml:
                    smg_content = smg_xxml.read()
            except FileNotFoundError:
        ...
        if isfile(join(smg_dir, "product.smgxml")):
            with open(clean_path(join(smg_dir, "product.smgXml")), "w", encoding="utf-8") as smg_xxml:
                smg_xxml.write(new_sgm_content)
        elif isfile(join(smg_dir, filename, "product.smgXml")):
            with open(clean_path(join(smg_dir, filename, "product.smgXml")), "w", encoding="utf-8") as smg_xxml:
                smg_xxml.write(new_sgm_content)
        else:
            if qt_window is not None:
                progress.emit(100)
