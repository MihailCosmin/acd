...
def pdf_to_dictionary(first_page: int, last_page: int, output_folder: str, top_x: int, top_y: int, bottom_x: int, bottom_y: int):
    ...
    if not isfile(join(expanduser("~/Desktop"), "pdf_dictionary_part.txt")):
        with open(clean_path(join(expanduser("~/Desktop"), "pdf_dictionary_part.txt")), "w") as f:
            f.write("[\n")
    ...
    while page < last_page + 1:
        try:
            ...
            for ind, pos in enumerate(positions[:-1]):
                ...
                with open(clean_path(join(expanduser("~/Desktop"), "pdf_dictionary_part.txt")), "a") as f:
    ...
    with open(clean_path(join(expanduser("~/Desktop"), "pdf_dictionary_part.txt")), "a") as f:
        f.write("\n]")
    with open(clean_path(join(expanduser("~/Desktop"), "pdf_dictionary.txt")), "w") as f:
...
if __name__ == "__main__":
    ...
    # with open(clean_path(join(expanduser("~/Desktop"), "dictionary.txt")), "w") as f:
    #     f.write(str(dictionary))
