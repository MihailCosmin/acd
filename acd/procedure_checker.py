    ...
    def check(self):
        ...
        if self.debug:
            if isinstance(self.qt_window, QMainWindow):
                ...
                with open(clean_path(join(self.qt_window.exe_path, "debug", "dict_ipl_table.json")), "w", encoding="utf-8") as log_file:
                    dump(dict_ipl_table, log_file, ensure_ascii=False, indent=4)
                with open(clean_path(join(self.qt_window.exe_path, "debug", "dict_disassembly.json")), "w", encoding="utf-8") as log_file:
                    dump(dict_disassembly, log_file, ensure_ascii=False, indent=4)
                with open(clean_path(join(self.qt_window.exe_path, "debug", "dict_repair.json")), "w", encoding="utf-8") as log_file:
                    dump(dict_repair, log_file, ensure_ascii=False, indent=4)
                with open(clean_path(join(self.qt_window.exe_path, "debug", "dict_assembly.json")), "w", encoding="utf-8") as log_file:
